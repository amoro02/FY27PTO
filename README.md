...
Checklist:
1. In Vercel project settings, set the connected branch to your production branch (recommended: `codex/develop-internal-web-app-for-pto-management-a2augr`).
...
## Fix for your current Vercel screen (404 + Source shows old commit)
If Vercel shows **Source: your production branch -> old initialize commit**, it means production is still pinned to the old commit that has no app files.

Do this exactly:
1. In GitHub, keep one production branch (your single branch `codex/develop-internal-web-app-for-pto-management-a2augr`) and ensure it contains commit `1ae3a31` or newer.
2. In Vercel -> Project -> **Settings -> Git**:
   - Production Branch = the branch that has the app code.
3. In Vercel -> Project -> **Settings -> General**:
   - Root Directory = `/` (repo root).
4. In Vercel -> Deployments:
   - Click **Redeploy** on latest deployment and choose **Use existing Build Cache: OFF**.
5. Verify Environment Variables are set in Vercel Production:
   - `DATABASE_URL` (Neon)
   - `JWT_SECRET`
   - `ADMIN_PASSCODE`

Optional hard reset if still stuck:
```bash
git checkout codex/develop-internal-web-app-for-pto-management-a2augr
git push origin codex/develop-internal-web-app-for-pto-management-a2augr --force-with-lease
```
Then trigger a fresh redeploy in Vercel.

## Branch Strategy (single branch)
For your requested workflow (GitHub -> Vercel -> Neon), keep only one long-lived branch:
- `codex/develop-internal-web-app-for-pto-management-a2augr` = the only active branch for production.
